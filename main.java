    ...
    activity.runOnUiThread(new Runnable() {
        public void run() {
            try {
                ...
                content.addView(createSectionTitle(activity, "MiMo 配置"));
                final EditText keyInput = createInputCard(activity, content, "MiMo API 密钥（sk- 开头）", getApiKey(), false);
                final EditText urlInput = createInputCard(activity, content, "API 地址", getBaseUrl(), false);
                final EditText modelInput = createInputCard(activity, content, "模型", getModel(), false);
